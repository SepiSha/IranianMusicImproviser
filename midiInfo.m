...
for i=1:length(tracklist)
    ...
    for msgNum=1:length(midi.track(tracknum).messages)
        ...
        if (midimeta==1 && type==144 && data(2)>0)
            % note on:
            Notes(end+1,:) = [tracknum chan data(1) data(2) seconds 0 msgNum -1 0 0 current_tempo 0];
        ...
        elseif (midimeta==1 && ( (type==144 && data(2)==0) || type==128 ))
            ...
            if (length(ind)==0)
                ...
                Notes(ind,6) = seconds;
                Notes(ind,8) = msgNum;
                %fprintf("1pitchbend amount=%d\n", 0)
            end
            
            %fprintf("2pitchbend amount=%d\n", type)
        ...
        if (type==PITCH_BEND);
            ...
            val2= data(1) + bitand( data(2),(kk))*128;
            
            if (data(2) < 64) %64 was 0x40
                val2 =val2-8192;
            end
            if (val2==0)
                val2=-1;
            end
            Notes(end+1,:) = [tracknum chan -3 0 seconds 0 msgNum -1 val2 0 current_tempo 0];
%data1 data2=0
        ...
        %=======
        GENERALPURPOSECONTROLLER=176;
        
        % find start/stop of gushes:
        
        if (type==GENERALPURPOSECONTROLLER);
            %fprintf("Type,Data1,Data2 %d %d %d\n", type, data(1),data(2));
            if ((data(1)==16) && (data(2)==127));
                Notes(end+1,:) = [tracknum chan -4 0 seconds 0 msgNum -1 0 GENERALPURPOSECONTROLLER 0 0];%data1,data2=0
            end
        end
        ...
        if (type==MEASURE)
            MeasureNumebr=MeasureNumebr+1;
            %fprintf("Type,Data1,Data2 %d %d %d %d\n", type, data(1),data(2),(data(1)*10000)+2^(data(2))+((10000^2)*MeasureNumebr) );
            %size(Notes)
            
            
            %Notes(end+1,:) = [tracknum 0 data(1) data(2) seconds bbb msgNum -1 0 MEASURE current_tempo  (data(1)*10000)+2^(data(2))+((10000^2)*MeasureNumebr)];
            Notes(end+1,:) = [tracknum 0 -5 0 seconds 0 -3 -5  0 MEASURE current_tempo  (data(1)*10000)+2^(data(2))+((10000^2)*MeasureNumebr)]; %data1 data2=0
