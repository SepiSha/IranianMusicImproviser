...
for i=1:Ntracks
  ...
  for j=1:length(ord)
    ...
    if mod(trM(n,3),2)==0  % for notes without microtone

        midi.track(i).messages(msgCtr).deltatime = cumticks - prevtick;
        %fprintf("msg.deltatime=%d %d %d\n", cumticks - prevtick,trM(n,3)/2, note_events_onoff(ord(j)))
        ...
        if (note_events_onoff(ord(j))==1)
            ...
            midi.track(i).messages(msgCtr).data = [trM(n,3)/2; trM(n,4)];
        else
            %-- note off msg:
            %midi.track(i).messages(msgCtr).type = 128;
            %midi.track(i).messages(msgCtr).data = [trM(n,3)/2; trM(n,4)];
            %-- note on vel=0:
            midi.track(i).messages(msgCtr).type = 144;  % change this to Note_OFF
            midi.track(i).messages(msgCtr).data = [trM(n,3)/2; 0];
        end
        msgCtr = msgCtr + 1;
    
        prevtick = cumticks;
    elseif (1)  % odd note: need a pitch bend:

        midi.track(i).messages(msgCtr).deltatime = cumticks - prevtick;
        %fprintf("---------PB msg.deltatime=%d %d %d\n", cumticks - prevtick,trM(n,3)/2, note_events_onoff(ord(j)))
        midi.track(i).messages(msgCtr).midimeta = 1;
        midi.track(i).messages(msgCtr).chan = trM(n,2);
        midi.track(i).messages(msgCtr).used_running_mode = 0;
        ...
        if (note_events_onoff(ord(j))==1)
            ...
            midi.track(i).messages(msgCtr).type = 144;
            midi.track(i).messages(msgCtr).data = [(trM(n,3)+1)/2; trM(n,4)];
        else
            %-- note off msg: -- note on vel=0:
            midi.track(i).messages(msgCtr).type = 144;
            midi.track(i).messages(msgCtr).data = [(trM(n,3)+1)/2; 0];

            %We add a pitchBendOff here
        end
         msgCtr = msgCtr + 1;
%Now we make the pitchbend message (with the same timing as note_on note_off):
        midi.track(i).messages(msgCtr).deltatime = 0;  %cumticks - prevtick;
        %fprintf("--------  Pitchbend msg.deltatime=%d %d %d\n", cumticks - prevtick,trM(n,3)/2, note_events_onoff(ord(j)))
        midi.track(i).messages(msgCtr).midimeta = 1;
        midi.track(i).messages(msgCtr).chan = trM(n,2);
        midi.track(i).messages(msgCtr).used_running_mode = 0;
        ...
        if (note_events_onoff(ord(j))==1)
            ...
            midi.track(i).messages(msgCtr).data = [0; 48];  %-2048
        else
            %--  pitchBend off msg: -- note on vel=0:
            midi.track(i).messages(msgCtr).type = 224;
            midi.track(i).messages(msgCtr).data = [0; 64];  % pitchbend=0
